#!/usr/bin/env python3
"""P14 regression tests for atomic state, pure validation, and replay evidence."""
...
from adapters.feishu.team_cli import JsonFileStateStore
...
class FeishuStateMigrationTest(unittest.TestCase):
    def test_legacy_cli_state_migrates_once_to_atomic_project_snapshots(self) -> None:
        with tempfile.TemporaryDirectory(prefix="fde-feishu-state-") as temp:
            legacy_path = Path(temp) / "feishu-team-state.json"
            legacy_path.write_text(
                json.dumps({"p1": {"project": {"status": "intake"}}}),
                encoding="utf-8",
            )

            store = JsonFileStateStore(legacy_path)
            self.assertEqual(store.get("p1", "project"), {"status": "intake"})
            self.assertEqual(store.root, legacy_path.with_suffix(".d").resolve())
            self.assertTrue(any(store.projects_dir.glob("*.json")))

            store.set("p1", "project", {"status": "active"})
            reopened = JsonFileStateStore(legacy_path)
            self.assertEqual(reopened.get("p1", "project"), {"status": "active"})


class UsageAccountingTest(unittest.TestCase):
    def test_worker_dispatch_records_tool_usage_in_host_code(self) -> None:
        usage = []
        result = call_worker_agent_wrapper(
            "call_echo_agent",
            {},
            {},
            lambda _agent_id: "role card",
            lambda _prompt, _args: {"status": "done"},
            usage_recorder=lambda **item: usage.append(item),
        )
